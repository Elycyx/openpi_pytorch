...
class Embedder(nn.Module):
    ...
    def decode(self, x: torch.Tensor) -> torch.Tensor:
        return functional.linear(x, self.embedding.weight)
...
class Attention(nn.Module):
        ...
        for config in configs:
            ...
            if lora_cfg is not None:
                if config.num_kv_heads == config.num_heads:
                    self.q_proj.append(
                        lora.Einsum(
                            (3, config.num_heads, config.width, config.head_dim),
                            "BSD,QKDH->QBSKH",
            ...
            elif config.num_kv_heads == config.num_heads:
    ...
    def forward(
        ...
        for i, x in enumerate(xs):
            ...
            if isinstance(self.q_proj[i], lora.Einsum):
                projected = self.q_proj[i](x)
                if self.k_proj[i] is None:
                    q, k, v = projected.unbind(dim=0)
                else:
                    q = projected
                ...
                q = self.q_proj[i](x).reshape(batch_size, sequence_length, self.num_heads, self.head_dim)
        ...
        queries_per_kv_head = self.num_heads // num_kv_heads

        q_r = q.reshape(q.shape[0], q.shape[1], num_kv_heads, queries_per_kv_head, self.head_dim)
        k_r = k.reshape(k.shape[0], k.shape[1], num_kv_heads, self.head_dim)
        v_r = v.reshape(v.shape[0], v.shape[1], num_kv_heads, self.head_dim)
        ...
        probs = functional.softmax(masked_logits, dim=-1).to(dtype)
        ...
        for i, x in enumerate(xs):
            if x is not None:
                ...
                expert_out = encoded[:, start:end]
                if not isinstance(self.o_proj[i], lora.Einsum):
                    expert_out = expert_out.reshape(x.shape[0], x.shape[1], -1)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.matmul(activations, self.w_linear.to(dtype))
...
class Block(nn.Module):
    ...
    def forward(
        ...
        out = []
        gates = []
        for i, (x, _config) in enumerate(zip(xs, self.configs, strict=True)):
            if x is not None:
