    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        for layer in self.layers:
            if self.gradient_checkpointing and self.training:
                x = torch.utils.checkpoint.checkpoint(layer, x, use_reentrant=self.gradient_checkpointing_use_reentrant)
            else:
                x = layer(x)
...
class SigLIPViT(nn.Module):
    ...
    def forward(self, image: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        x = x.reshape(batch_size, channels, h * w).permute(0, 2, 1)  # (B, h*w, width)

        # Add position embedding in float32
        x = x + self.pos_embedding.float()

        if self.pool_type == "tok":
            cls_tokens = self.cls_token.float().expand(batch_size, -1, -1)
